...
# Parse product names from the YOLO model output and extract price/weight
product_list = """
...
def extract_price_and_weight(product_name):
    """Extract price and weight from product name"""
    # Pattern: something_WEIGHTunit_PRICErs_suffix
    # Examples: balaji_wafers_masalamasti_35g_10rs_front
    #           tata_salt_1kg_28rs_back
    #           dove_shampoo_180ml_150rs_back
    
    # Extract price (numbers before 'rs')
    price_match = re.search(r'_(\d+)rs_', product_name)
    price = int(price_match.group(1)) if price_match else 0
    
    # Extract weight with unit
    weight_match = re.search(r'_(\d+(?:\.\d+)?)(g|kg|ml|l)_', product_name)
    ...
    if weight_match:
        ...
        # Convert to kg
        if weight_unit == 'g':
            weight = weight_value / 1000
        elif weight_unit == 'ml':
            weight = weight_value / 1000  # Treat ml as liters for now
        elif weight_unit == 'l':
            weight = weight_value
        else:  # kg
            weight = weight_value
    ...
    return price, round(weight, 3)

# Generate items dictionary
items = {}
products = [p.strip() for p in product_list.strip().split('\n') if p.strip()]

for product in products:
    price, weight = extract_price_and_weight(product)
    items[product] = [price, weight]

# Save to JSON file
with open('static/data/items.json', 'w') as f:
    json.dump(items, f, indent=4)

print(f"✓ Generated items.json with {len(items)} products")
print(f"\nSample entries:")
for i, (key, value) in enumerate(list(items.items())[:5]):
    print(f"  {key}: Rs.{value[0]}, {value[1]}kg")
print("  ...")